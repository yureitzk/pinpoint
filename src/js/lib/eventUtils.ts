...
export function trackDraggingMovement(
	coords: { x: number; y: number },
...
export function isInClickCooldown(
...
export function handleDragEnd(
	...
	const distance = calculateDistance(
		coords,
		pointerState.lastPointerDownPosition,
	);
	const isWithinCooldown = isInClickCooldown(
		pointerState.lastClickTime,
		clickCooldownMs,
	);

	if (
		distance < dragThreshold &&
		!pointerState.hasMovedWhileDragging &&
		!isWithinCooldown
	) {
...
export function cleanupDragging(
	pointerState: PointerState,
	canvas: HTMLCanvasElement,
	e: PointerEvent,
): void {
